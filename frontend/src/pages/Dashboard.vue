  ...
  <div class="space-y-8 pb-10">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
      ...
      >
        ...
        <div class="relative z-10 flex flex-col justify-between h-full">
          <div class="p-2.5 sm:p-3 rounded-xl w-fit mb-4 sm:mb-6 shadow-sm" :class="[stat.bg, stat.color]">
            <component :is="stat.icon" :size="20" />
          </div>
          <div>
            <p class="text-[9px] sm:text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1">{{ stat.title }}</p>
            <div class="flex items-baseline gap-1.5 sm:gap-2">
              <h3 class="text-2xl sm:text-3xl font-extrabold text-gray-800 tracking-tight">
                ...
                <template v-else>{{ stat.val }}</template>
              </h3>
    ...
    >
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        >
          <div class="w-10 h-10 rounded-lg bg-kementan-green/10 flex items-center justify-center text-kementan-green group-hover:bg-kementan-green group-hover:text-white transition-all shrink-0">
            <FileText :size="18" />
          </div>
          <div>
            <p class="text-sm font-bold text-gray-800">Surat Tugas</p>
            <p class="text-[10px] text-gray-500 font-medium">Kelola SPT Direktorat</p>
          </div>
        </router-link>
        <router-link 
          to="/sptjm" 
        ...
        >
          <div class="w-10 h-10 rounded-lg bg-blue-500/10 flex items-center justify-center text-blue-600 group-hover:bg-blue-600 group-hover:text-white transition-all shrink-0">
            <FileEdit :size="18" />
          </div>
          <div>
            <p class="text-sm font-bold text-gray-800">SPTJM</p>
            <p class="text-[10px] text-gray-500 font-medium">Biaya Rill Perjalanan</p>
          </div>
        </router-link>
        <router-link 
          to="/pegawai" 
        ...
        >
          <div class="w-10 h-10 rounded-lg bg-orange-500/10 flex items-center justify-center text-orange-600 group-hover:bg-orange-600 group-hover:text-white transition-all shrink-0">
            <Users :size="18" />
          </div>
          <div>
            <p class="text-sm font-bold text-gray-800">Master Pegawai</p>
            <p class="text-[10px] text-gray-500 font-medium">Database Personel</p>
          </div>
        </router-link>
      </div>
    </div>
...
import { 
  ...
  Calendar as CalendarIcon, FileEdit 
...
onMounted(async () => {
  ...
  // Fetch real counts (lightweight — reuse existing APIs)
  try {
    const tim_poksi = adminData.tim_poksi || ''
    const [resSpt, resSptjm] = await Promise.all([
    ...
    const [dataSpt, dataSptjm] = await Promise.all([resSpt.json(), resSptjm.json()])
    
    if (dataSpt.success) {
      sptCount.value = dataSpt.data.length
      sptWithPdf.value = dataSpt.data.filter(s => s.file_link).length
    }
    if (dataSptjm.success) {
      sptjmCount.value = dataSptjm.data.length
      sptjmWithPdf.value = dataSptjm.data.filter(s => s.file_link).length
    }
  } catch (err) {
    // Silent fail — stats just show 0
  } finally {
...
const stats = computed(() => [
  { title: 'Total SPT', val: String(sptCount.value).padStart(2, '0'), icon: FileText, color: 'text-kementan-green', bg: 'bg-kementan-green/10', border: 'border-kementan-green/20' },
  { title: 'SPT Selesai', val: String(sptWithPdf.value).padStart(2, '0'), icon: CheckCircle, color: 'text-emerald-500', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20' },
  { title: 'Total SPTJM', val: String(sptjmCount.value).padStart(2, '0'), icon: FileEdit, color: 'text-blue-500', bg: 'bg-blue-500/10', border: 'border-blue-500/20' },
  { title: 'SPTJM Selesai', val: String(sptjmWithPdf.value).padStart(2, '0'), icon: CheckCircle, color: 'text-orange-500', bg: 'bg-orange-500/10', border: 'border-orange-500/20' },
])
</script>
